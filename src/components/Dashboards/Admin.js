...
import {getCausesForAcception, savecause, deleteCauses, getCauseByID} from '@api';
...
let EnhancedTableToolbar = props => {
  const { numSelected, classes, onDeleteItem, onManageItem} = props;
  ...
  return (
    ...
    >
      ...
      <div className={classes.actions}>
        {numSelected ==1 && (
          <Tooltip title="Manage a cause">
            <IconButton aria-label="Edit" onClick={event=>onManageItem(event)}>
              <SettingIcon />
            </IconButton>
          </Tooltip>
        )}  
...
export class CausesList extends PureComponent {
    ...
    if(confirm('Are you sure?')){
      ...
      call.then((res)=>{
        if (res.msg == msg.SUCCESS){
          self.state.causes.forEach((item)=>{
            if (item.id==id){
  ...
  handleApprove = (event, id)=>{
    ...
    this.updateCausseStatus(id,"approve");
  }
  handleDeny = (event, id)=>{
    event.stopPropagation();
    this.updateCausseStatus(id,"deny");
  }
  ...
  handleDeleteCauses = event=>{
    if(confirm("Are you delete the item(s)?")){
      const self = this;
  ...
  handleManageCause = (event,id)=>{
    const self = this;
    const {addCause,push} = this.props;
    event.stopPropagation();
    
    getCauseByID({id:id?id:this.state.selected[0]}).then((res)=>{
      if (res.msg==msg.SUCCESS&& res.cause){
        addCause(CauseModel.fromJS(res.cause));
        push('/admin-editcause'); 
      }
    });
  }
  updateCauses(){
        ...
        <div className="root main-container">
            <Paper className={classes.root}>
                <EnhancedTableToolbar 
                  ...
                  onManageItem={this.handleManageCause}
                  />
                    ...
                    {!this.state.isLoadingItems&& stableSort(causes, getSorting(order, orderBy))
                        ...
                        return (
                            <TableRow
                                ...
                                <TableCell>
                                  {n.status!="approve"&&<Button variant="raised" className={classes.button} onClick={event=>this.handleApprove(event, n.id)}>Approve</Button>}
                                  <Button variant="raised" className={classes.button} onClick={event=>this.handleDeny(event, n.id)}>Deny</Button>
                                  <Button variant="raised" className={classes.button} onClick={event=>this.handleManageCause(event, n.id)}>Edit</Button>
                                </TableCell>
                            </TableRow>
                          );
                        })}
